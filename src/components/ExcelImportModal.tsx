...
export const ExcelImportModal: React.FC<ExcelImportModalProps> = ({
  ...
  // Load sample demo in 1-click
  const handleLoadSample = (sampleType: 'journal' | 'releve') => {
    setIsProcessing(true);
    setFileName(sampleType === 'journal' ? 'Journal_Ventes_Achats_Cabinet.xlsx' : 'Releve_MobileMoney_Wave_OM.xlsx');
    setFileSize('14.2 Ko');

    const sampleRows = sampleType === 'journal' ? [
      {
        'Date': '2026-09-02',
        'N° Pièce': 'FAC-2026-081',
        'Libellé de l\'opération': 'Vente de 20 sacs de riz parfumé',
        'Compte Débit': '5711',
        'Compte Crédit': '7011',
        'Montant TTC': 185000,
      ...
      {
        'Date': '2026-09-03',
        'N° Pièce': 'FAC-2026-082',
        'Libellé de l\'opération': 'Prestation maintenance réseau informatique',
        'Compte Débit': '5263',
        'Compte Crédit': '7061',
        'Montant TTC': 250000,
      ...
      },
      {
        'Date': '2026-09-04',
        'N° Pièce': 'BE-2026-041',
        'Libellé de l\'opération': 'Facture électricité CIE magasin',
        'Compte Débit': '6051',
        'Compte Crédit': '5261',
        'Montant TTC': 42500,
      ...
      },
      {
        'Date': '2026-09-05',
        'N° Pièce': 'LOY-09-2026',
        'Libellé de l\'opération': 'Loyer commercial boutique principale',
        'Compte Débit': '6221',
        'Compte Crédit': '5211',
        'Montant TTC': 300000,
      ...
      },
      {
        'Date': '2026-09-06',
        'N° Pièce': 'FOURN-109',
        'Libellé de l\'opération': 'Achat de 50 sacs de ciment Dangote',
        'Compte Débit': '6011',
        'Compte Crédit': '5711',
        'Montant TTC': 260000,
        'Moyen de Paiement': 'Espèces'
      },
      {
        'Date': '2026-09-07',
        'N° Pièce': 'CARB-008',
        'Libellé de l\'opération': 'Carburant et vidange camionnette livraison',
        'Compte Débit': '6053',
        'Compte Crédit': '5263',
        'Montant TTC': 35000,
        'Moyen de Paiement': 'Wave'
    ...
    ] : [
      {
        'Date': '2026-09-01',
        'Référence': 'WAV-9912',
        'Libellé de la Transaction': 'Règlement client Aminata Diop - Vente tissus',
        'Débit (Dépenses)': '',
        'Crédit (Recettes)': 175000,
        'Canal / Moyen': 'Wave'
      },
      {
        'Date': '2026-09-02',
        'Référence': 'OM-4410',
        'Libellé de la Transaction': 'Achat fournitures de bureau et ramettes',
        'Débit (Dépenses)': 32000,
        'Crédit (Recettes)': '',
        'Canal / Moyen': 'Orange Money'
      },
      {
        'Date': '2026-09-03',
        'Référence': 'VIR-7721',
        'Libellé de la Transaction': 'Encaissement virement client Société BTP',
        'Débit (Dépenses)': '',
        'Crédit (Recettes)': 620000,
        'Canal / Moyen': 'Banque'
      },
      {
        'Date': '2026-09-04',
        'Référence': 'MTN-8819',
        'Libellé de la Transaction': 'Frais forfait Internet MTN Fibre Pro',
        'Débit (Dépenses)': 45000,
        'Crédit (Recettes)': '',
        'Canal / Moyen': 'MTN MoMo'
      },
      {
        'Date': '2026-09-05',
        'Référence': 'ESP-0091',
        'Libellé de la Transaction': 'Vente comptant quincaillerie générale',
        'Débit (Dépenses)': '',
        'Crédit (Recettes)': 290000,
        'Canal / Moyen': 'Espèces'
      }
    ];

    setAvailableSheets(['Feuille_1']);
    setSelectedSheet('Feuille_1');
    setAllRowsBySheet({ 'Feuille_1': sampleRows });
    const detectedHeaders = Object.keys(sampleRows[0]);
    setHeaders(detectedHeaders);
    const initialMapping = detectColumns(detectedHeaders);
    setColumnMapping(initialMapping);
    buildPreview(sampleRows, initialMapping);
    setStep('preview');
    setIsProcessing(false);
  };
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {step === 'upload' && (
            <div className="space-y-6">
              ...
              <div className="bg-white border border-[#DDD6FE] p-5 rounded-2xl shadow-2xs space-y-3">
                <div className="flex items-center gap-2 text-xs font-bold text-[#1E084A]">
                  <Sparkles className="w-4 h-4 text-[#7024E3]" />
                  <span>Pas de fichier sous la main ? Testez avec nos jeux d'essai réels :</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <button
                    onClick={() => handleLoadSample('journal')}
                    className="p-3.5 text-left border border-[#DDD6FE] hover:border-[#7024E3] bg-[#FAF8FF] hover:bg-[#F5F3FF] rounded-xl transition-all group"
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-bold text-[#1E084A] group-hover:text-[#7024E3]">
                        Exemple : Journal SYSCOHADA Révisé
                      </span>
                  ...
                  <button
                    onClick={() => handleLoadSample('releve')}
                    className="p-3.5 text-left border border-[#DDD6FE] hover:border-[#7024E3] bg-[#FAF8FF] hover:bg-[#F5F3FF] rounded-xl transition-all group"
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-bold text-[#1E084A] group-hover:text-[#7024E3]">
                        Exemple : Relevé Wave / Orange Money & Banque
                      </span>
                      <ArrowRight className="w-4 h-4 text-[#7024E3] opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                    <p className="text-[11px] text-[#7C709A]">
